                       ...
                       double tolerance, int frameFreq)
{
     ...
     *img.getPixel(x,y) = fillColor(x,y);
     ...
     while(!sx.isEmpty()){
      ...
      if(xc+1<img.width() && yc-1>=0 && !check[xc+1][yc-1]
          && center.dist(*img.getPixel(xc+1,yc-1))<=tolerance){
           sx.add(xc+1);
           sy.add(yc-1);
           *img.getPixel(xc+1,yc-1) = fillColor(xc+1,yc-1);
           check[xc+1][yc-1] = true;
           freq++;
           if(freq == frameFreq){
             newAnimation.addFrame(img);
             freq = 0;
      ...
      if(yc-1>=0 && !check[xc][yc-1] && center.dist(*img.getPixel(xc,yc-1))<=tolerance){
           sx.add(xc);
           sy.add(yc-1);
           *img.getPixel(xc,yc-1) = fillColor(xc,yc-1);
           check[xc][yc-1] = true;
           freq++;
           if(freq == frameFreq){
             newAnimation.addFrame(img);
      ...
      if(xc-1>=0 && yc-1>=0 && !check[xc-1][yc-1]
        && center.dist(*img.getPixel(xc-1,yc-1))<=tolerance){
           sx.add(xc-1);
           sy.add(yc-1);
           *img.getPixel(xc-1,yc-1) = fillColor(xc-1,yc-1);
      ...
      if(xc-1>=0 && !check[xc-1][yc] && center.dist(*img.getPixel(xc-1,yc))<=tolerance){
           sx.add(xc-1);
           sy.add(yc);
           *img.getPixel(xc-1,yc) = fillColor(xc-1,yc);
           check[xc-1][yc] = true;
           freq++;
           if(freq == frameFreq){
             newAnimation.addFrame(img);
      ...
      if(xc-1>=0 && yc+1<img.height() && !check[xc-1][yc+1]
        && center.dist(*img.getPixel(xc-1,yc+1))<=tolerance){
           sx.add(xc-1);
           sy.add(yc+1);
           *img.getPixel(xc-1,yc+1) = fillColor(xc-1,yc+1);
      ...
      if(yc+1<img.height() && !check[xc][yc+1] && center.dist(*img.getPixel(xc,yc+1))<=tolerance){
           sx.add(xc);
           sy.add(yc+1);
           *img.getPixel(xc,yc+1) = fillColor(xc,yc+1);
           check[xc][yc+1] = true;
      ...
      if(xc+1<img.width() && yc+1<img.height() && !check[xc+1][yc+1]
          && center.dist(*img.getPixel(xc+1,yc+1))<=tolerance){
           sx.add(xc+1);
           sy.add(yc+1);
           *img.getPixel(xc+1,yc+1) = fillColor(xc+1,yc+1);
      ...
      if(xc+1<img.width() && !check[xc+1][yc] && center.dist(*img.getPixel(xc+1,yc))<=tolerance){
           sx.add(xc+1);
           sy.add(yc);
           *img.getPixel(xc+1,yc) = fillColor(xc+1,yc);
           check[xc+1][yc] = true;
